...
def generate_mm(T, seed=42, wait_coef=0.5, arr_coef=1.0):
    ...
    for i in g.nodes:
        if i[1] == "h":
            w[i] = 1 * np.random.normal(loc=1.0, scale=0.1) * wait_coef
        elif i[1] == "m":
            w[i] = 1.5 * np.random.normal(loc=1.0, scale=0.1) * wait_coef
        else:
...
if __name__ == '__main__':
    '''
    below is the results of instance 1 in Table D.8 in the appendix
    '''

    seed = 0 
    L = 2
    N = 100
    wait_coef = 2.0 # high waiting cost
    instance = generate_mm(100, seed, wait_coef)
    instance.generate_cycles(L)
    sim = simulator(instance, L)
    sim.run(N)
    print(sim.results)