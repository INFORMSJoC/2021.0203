...
from helper import prob_alloc_cg
import copy
import math
...
class affine_model:
    ...
    def prob_alloc(self):
        ...
        return action, num_of_cg_cuts

    def prob_alloc_alt(self):
        # non-bipartite with L=2
        if not self.optimized:
            self.optimize()
        G = nx.Graph()
        x0 = {}
        s0 = {}
        for xx in self.x:
            if xx[0] == 1 and self.x[xx].X > 1e-6:
                if xx[1] not in G.nodes:
                    G.add_node(xx[1])
                    s0[xx[1]] = self.DM.s[xx[1]]
                if xx[2] not in G.nodes:
                    G.add_node(xx[2])
                    s0[xx[2]] = self.DM.s[xx[2]]
                G.add_edge(xx[1], xx[2])
                x0[(xx[1], xx[2])] = self.x[xx].X
        m = prob_alloc_cg_alt(G, x0, s0)
        action = {}
        for c in self.Cycles:
            if c in m:
                action[c] = m[c]
    ...
    def prob_solve_alt(self):
        action = self.prob_alloc_alt()
        reward = 0
        for i in self.DM.Nodes:
            reward -= self.DM.s[i] * self.w[i]
        for c in self.Cycles:
